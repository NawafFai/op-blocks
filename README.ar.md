...
## اختيار حزمة الخصائص (اقرأها مرة واحدة)

تأخذ البلوكات **كل** الثيرموديناميكا من حزمة الخصائص التي تختارها، فاختر ما
يمثّل تغذيتك:

| التغذية | Aspen Plus | DWSIM |
|---|---|---|
| ماء عذب / مخفف | IDEAL | Ideal / NRTL |
| مياه بحر، محاليل ملحية، إلكتروليتات | **ELECNRTL** (قالب ONE PROCESS المرفق يستخدمه أصلًا) | حزمة مائية / إلكتروليتية أصلية |
| ⛔ ممنوعة للتغذية الملحية | STEAMNBS / STEAM-TA (جداول بخار الماء النقي) | Steam Tables |

سلوكان يستحقان المعرفة:

- إذا لم يتجاوز الضغط المطبق الضغطَ الأسموزي للتغذية، يكتمل البلوك الأسموزي
  **بنفاذ صفري وتحذير واضح** — هذه فيزياء صادقة وليست خطأ. ارفع الضغط أو خفف
  التغذية.
- التغذية الخارجة عن الغلاف الفيزيائي للعملية (مثل RO على محلول شبه مشبع —
  ‏NaCl يتشبع قرب 26 wt%) تُرفض مبكرًا برسالة تذكر الرقم والحد والمعدات
  الصحيحة البديلة (مبخّر / مبلوِر) — لا خطأ مضيف غامض أبدًا.
